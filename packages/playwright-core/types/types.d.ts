...
export const proxy: ProxyFactory;
...
export interface Proxy {
  /**
   * Emitted when a request passes through the proxy. The [request] object is read-only. In order to intercept and
   * mutate requests, see
   * [proxy.route(url, handler[, options])](https://playwright.dev/docs/api/class-proxy#proxy-route).
   */
  on(event: 'request', listener: (request: Request) => any): this;
   ...
   *
   * To remove a route with its handler you can use
   * [proxy.unroute(url[, handler])](https://playwright.dev/docs/api/class-proxy#proxy-unroute).
   * @param url A glob pattern, regex pattern or predicate receiving [URL] to match while routing. When a
   * [`baseURL`](https://playwright.dev/docs/api/class-browser#browser-new-context-option-base-url) via the context
   * options was provided and the passed URL is a path, it gets merged via the
   * [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) constructor.
  ...
  /**
   * Removes a route created with
   * [proxy.route(url, handler[, options])](https://playwright.dev/docs/api/class-proxy#proxy-route). When
   * [`handler`](https://playwright.dev/docs/api/class-proxy#proxy-unroute-option-handler) is not specified, removes all
   * routes for the [`url`](https://playwright.dev/docs/api/class-proxy#proxy-unroute-option-url).
   * @param url A glob pattern, regex pattern or predicate receiving [URL] used to register a routing with
   * [proxy.route(url, handler[, options])](https://playwright.dev/docs/api/class-proxy#proxy-route).
   * @param handler Optional handler function used to register a routing with
   * [proxy.route(url, handler[, options])](https://playwright.dev/docs/api/class-proxy#proxy-route).
...
/**
 * This class is used for creating [Proxy](https://playwright.dev/docs/api/class-proxy) instances which in turn can be
 * used to intercept network traffic from your application server. An instance of this class can be obtained via
 * [playwright.proxy](https://playwright.dev/docs/api/class-playwright#playwright-proxy). For more information see
 * [Proxy](https://playwright.dev/docs/api/class-proxy).
 */
export interface ProxyFactory {
  /**
   * Creates a new instance of [Proxy](https://playwright.dev/docs/api/class-proxy).
   * @param options
   */
  newProxy(options?: {
    /**
